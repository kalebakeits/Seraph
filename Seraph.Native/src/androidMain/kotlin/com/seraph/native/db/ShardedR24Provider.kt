...
/**
 * Read-only wrapper around R24 queries that transparently handles sharded cold data.
 *
 * Hot data (current month + previous month) is queried directly from the main DB via R24Dao.
 * Cold data (older months) is unzipped from shard files, queried, then cleaned up.
 *
 * Callers use this instead of R24Dao for any historical date-range query.
 * All writes still go through R24Dao directly.
 */
class ShardedR24Provider(
    ...
    fun queryByDate(date: String): List<R24> {
        ...
        return if (isHot(localDate)) {
            ...
            queryFromShard(YearMonth.from(localDate)) { shardDao ->
                shardDao.queryByDate(date)
            }
        }
    }
    ...
    fun queryFromTs(
        ...
        return if (isHot(localDate)) {
            ...
            queryFromShard(YearMonth.from(localDate)) { shardDao ->
                shardDao.queryFromTs(date, fromTs)
            }
        }
    }
    ...
    fun queryByDateRange(
        ...
        if (startMonth == endMonth) {
            ...
            return if (isHot(date)) {
                ...
                queryFromShard(startMonth) { shardDao ->
                    shardDao.queryByDateRange(startMs, endMs)
                }
            }
        }

        // Spans multiple months — split into hot/cold, merge results
        ...
        while (month <= endMonth) {
            ...
            val sampleDate = month.atDay(1)
            if (isHot(sampleDate)) {
                results += r24Dao.queryByDateRange(qStart, qEnd)
            } else {
                results +=
                    queryFromShard(month) { shardDao ->
                        shardDao.queryByDateRange(qStart, qEnd)
                    }
            }
    ...
    private fun isHot(date: LocalDate): Boolean {
        ...
        val hotBoundary = YearMonth.from(today).minusMonths(1).atDay(1)
    ...
    private fun queryFromShard(
        ...
        return try {
            ...
            val shardDb = openReadOnlyDb(tmpFile)
            val shardDao = R24Dao(shardDb)
            query(shardDao)
        } finally {
            tmpFile.delete()
        }
    ...
    private fun openReadOnlyDb(file: File): SeraphDb {
        val driver =
            AndroidSqliteDriver(
...
private fun java.time.LocalDate.toEpochMs(): Long = this.atStartOfDay(java.time.ZoneOffset.UTC).toInstant().toEpochMilli()
